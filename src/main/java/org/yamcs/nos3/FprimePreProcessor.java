 ...
 * </pre>
 */
// dead beef 0000 0008 0000 0f01
public class FprimePreProcessor extends CcsdsPacketPreprocessor {
    static final int MINIMUM_LENGTH = 12;

    public FprimePreProcessor(String yamcsInstance) {
        this(yamcsInstance, YConfiguration.emptyConfig());
    }

    public FprimePreProcessor(String yamcsInstance, YConfiguration config) {
    ...
    public TmPacket process(TmPacket pkt) {
        ...
        // system.out.println("packet is : %d");
        ...
        for (byte b : packet) {
            hexString.append(String.format("%02X", b & 0xFF)); // Convert to uppercase hex
        }

        eventProducer.sendWarning("packet is "+ hexString);
        
            // }

        // System.out.println("zachs loop");
        // for (int i=0; i<=packet.length ; i++){
        //     System.out.print(packet[i] + " "); // Output: 65 66 67
        // }
        // System.out.println ();
        ...
        for (byte b : packet) {
            hexString2.append(String.format("%02X", b & 0xFF)); // Convert to uppercase hex
        }
        eventProducer.sendWarning("packet after processing is "+ hexString2);
        return pkt;
    }
